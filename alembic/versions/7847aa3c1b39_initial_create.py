...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "ensemble_name", "version", name="ensemble_name_version_key"
        ),
    )
    op.create_table(
        "grids",
    ...
    with op.batch_alter_table("climatological_times", schema=None) as batch_op:
        batch_op.create_index(
            "climatological_times_time_set_id_key", ["time_set_id"], unique=False
        )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["grid_id"],
            ["grids.grid_id"],
            name="y_cell_bounds_grid_id_fkey",
            ondelete="CASCADE",
        ),
    ...
    op.create_table(
        "data_file_variables_qc_flags",
        sa.Column("data_file_variable_id", sa.Integer(), nullable=False),
        sa.Column("qc_flag_id", sa.Integer(), nullable=False),
        sa.ForeignKeyConstraint(
            ["data_file_variable_id"],
