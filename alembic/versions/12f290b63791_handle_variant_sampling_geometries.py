...
def copy_level_set_id_and_grid_id_to_data_file_variables_gridded():
    ...
    dfvs = sa.Table("data_file_variables", sa.MetaData(), autoload_with=op.get_bind())
    dfvs_gridded = sa.Table(
        "data_file_variables_gridded", sa.MetaData(), autoload_with=op.get_bind()
    )
    op.get_bind().execute(dfvs.update().values(geometry_type="gridded"))
    op.get_bind().execute(
        dfvs_gridded.insert().from_select(
            ["id", "level_set_id", "grid_id"],
            sa.select(
                dfvs.c.data_file_variable_id, dfvs.c.level_set_id, dfvs.c.grid_id
            ),
        )
    )
...
def upgrade():
    ...
    op.create_table(
        "data_file_variables_dsg_time_series",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["id"], ["data_file_variables.data_file_variable_id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["level_set_id"],
    ...
    op.create_table(
        "data_file_variables_dsg_time_series_x_stations",
        sa.Column("data_file_variable_dsg_ts_id", sa.Integer(), nullable=False),
        sa.Column("station_id", sa.Integer(), nullable=False),
        sa.ForeignKeyConstraint(
            ["data_file_variable_dsg_ts_id"],
    ...
    with op.batch_alter_table("data_file_variables", schema=None) as batch_op:
        ...
        batch_op.drop_constraint(
            "data_file_variables_level_set_id_fkey", type_="foreignkey"
        )
...
def copy_level_set_id_and_grid_id_from_data_file_variables_gridded():
    ...
    if get_dialect() == "sqlite":
        ...
        op.get_bind().execute(
            dfvs.update().values(
                level_set_id=sa.select(dfvs_gridded.c.level_set_id).where(
                    dfvs_gridded.c.id == dfvs.c.data_file_variable_id
                ),
                grid_id=sa.select(dfvs_gridded.c.grid_id).where(
                    dfvs_gridded.c.id == dfvs.c.data_file_variable_id
                ),
            )
        )
...
def downgrade():
    ...
    with op.batch_alter_table("data_file_variables", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("level_set_id", sa.INTEGER(), autoincrement=False, nullable=True)
        )
        batch_op.add_column(
            sa.Column(
                "grid_id",
        ...
        batch_op.create_foreign_key(
            "data_file_variables_level_set_id_fkey",
            "level_sets",
            ["level_set_id"],
            ["level_set_id"],
        )
        batch_op.create_foreign_key(
            "data_file_variables_grid_id_fkey", "grids", ["grid_id"], ["grid_id"]
        )
