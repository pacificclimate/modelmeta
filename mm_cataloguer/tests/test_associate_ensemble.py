...
from mm_cataloguer.associate_ensemble import find_ensemble, associate_ensemble_to_cf, associate_ensemble_to_files
...
def test_find_ensemble(test_session_with_ensembles, ensemble1):
    assert ensemble1 == find_ensemble(test_session_with_ensembles, ensemble1.name, ensemble1.version)
    assert not find_ensemble(test_session_with_ensembles, 'foo', ensemble1.version)


def test_associate_ensemble_to_cf(test_session_with_ensembles, ensemble1, tiny_dataset):
    ...
    assoc_df = associate_ensemble_to_cf(sesh, tiny_dataset, ensemble1.name, ensemble1.version)
    assert assoc_df == data_file
    for data_file_variable in data_file.data_file_variables:
        ensembles = sesh.query(Ensemble).join(EnsembleDataFileVariables)\
...
def test_associate_ensemble_to_files(ensemble1):
    ...
    # Associate them
    assoc_data_files = associate_ensemble_to_files(filenames, dsn, ensemble1.name, ensemble1.version)
    assert all(adf and adf.filename == f for (adf, f) in zip(assoc_data_files, filenames))
