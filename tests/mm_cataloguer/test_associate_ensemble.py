...
from mm_cataloguer.index_netcdf import index_cf_file, find_update_or_insert_cf_file
...
def check_ensemble_associations(session, data_file_variables, ensemble):
    ...
    for data_file_variable in data_file_variables:
        ...
        assert any(
            ensemble.name == e.name and ensemble.version == e.version for e in ensembles
        )
...
def test_find_ensemble__(test_session_with_ensembles, ensemble1):
    ...
    assert not find_ensemble(test_session_with_ensembles, "foo", ensemble1.version)
...
@pytest.mark.parametrize(
    "tiny_gridded_dataset, var_names",
    [
        ("gcm", {"tasmax"}),
        ("downscaled", {"tasmax"}),
        (
...
def test_associate_ensemble_to_data_file_variable__(
    ...
    # Extract some info
    assert (
        set(dfv.netcdf_variable_name for dfv in data_file.data_file_variables)
        == var_names
    )
...
@pytest.mark.slow
@pytest.mark.parametrize(
    "tiny_gridded_dataset, var_names, expected_var_names",
    [
        ("gcm", None, {"tasmax"}),
        ("gcm", {"tasmax"}, {"tasmax"}),
...
def test_associate_ensemble_to_data_file__(
    ...
    # Verify
    assert (
        set(dfv.netcdf_variable_name for dfv in associated_dfvs) == expected_var_names
    )
...
def test_associate_ensemble_to_filepath__(
    ...
    # Verify
    assert set(df.filename for df, _ in associated_items) == set(
        str(ef) for ef in expected_filepaths
    )
...
@pytest.mark.slow
@pytest.mark.parametrize(
    "regex_filepath, filepaths, var_names, " "expected_filepaths, expected_var_names",
    [
...
def test_associate_ensemble_to_filepaths__(
    ...
    assert set(df.filename for df in associated_data_files) == expected_filepaths

    associated_data_file_variables = (
        session.query(DataFileVariable)
        .filter(
            DataFileVariable.id.in_(
    ...
    assert (
        set(dfv.netcdf_variable_name for dfv in associated_data_file_variables)
        == expected_var_names
    )
