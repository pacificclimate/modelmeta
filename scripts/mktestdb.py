...
def make_test():
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[1, 2],
        help="Schema version the database is using",
    )
    parser.set_defaults(ensemble="canada_map", version=2)
    ...
    if args.version == 1:
        ...
        if not args.dsn:
            args.dsn = (
                "postgresql://httpd_meta@monsoon.pcic.uvic.ca/pcic_meta?sslmode=require"
            )

        sequences = common_sequences + [
            ("variable_aliases", "variable_alias_id"),
            ("level_sets", "level_set_id"),
            ("qc_flags", "qc_flag_id"),
        ]
    ...
    q = rSession.query(modelmeta.Ensemble).filter(
        modelmeta.Ensemble.name == args.ensemble
    )
    ens = q.first()
