...
def copy():
    ...
    parser.add_argument(
        "-o",
        "--outdsn",
        help="Destination database DSN. Default is output.sqlite in current directory",
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    parser.set_defaults(outdsn="sqlite+pysqlite:///output.sqlite", version=2)
    ...
    if args.version == 1:
        ...
        if not args.dsn:
            args.dsn = (
                "postgresql://httpd_meta@monsoon.pcic/pcic_meta_v1?sslmode=require"
            )
