...
## This will end up being a two-step process. First, one must populate the database. Then, one can figure out all the interdependencies (derived_from, initialized_from, anomaly_from, etc).
...
create.data.file.variables <- function(f, data.file.id, con) {
  ...
  dfv.list <- lapply(var.list, function(v) {
    query <- paste("SELECT data_file_variable_id from data_file_variables where data_file_id=", data.file.id, " and netcdf_variable_name='", v, "';", sep="")
    ...
    if(nrow(data.file.variable.id) == 0) {
      ...
      return(paste("(", paste(data.file.id, variable.id, ifelse(is.null(variable.cell.methods), "NULL", shQuote(variable.cell.methods)), ifelse(is.null(level.set.id), "NULL", level.set.id), grid.id, shQuote(v), variable.range[1], variable.range[2], sep=","), ")", sep=""))
    }
  })
...
get.file.metadata.cmip3 <- function(f) {
  ...
  model.short.name <- ifelse(source.att$hasatt, strsplit(source.att$value, "[, \\(\\)\\{\\}:;]")[[1]][1], ifelse(run.att$hasatt, paste(strsplit(run.att$value, " ")[[1]][2:3], sep="_"), ''))
  ...
  expt.name.map <- c("(SRES B1)"="sresb1", "(SRES A2)"="sresa2", "(SRES A1B)"="sresa1b", "(20C3M)"="20c3m",
                     "IPCC SRES B1"="sresb1", "IPCC SRES A2"="sresa2", "IPCC SRES A1B"="sresa1b", "IPCC 20C3M"="20c3m")
  expt.name <- "OTHER"
  if(expt$hasatt) {
  ...
  run.att <- ncatt_get(f, 0, "run")
  source.att <- ncatt_get(f, 0, "source")
  run.name <- ifelse(realization.att$hasatt, paste("run", realization.att$value), ifelse(run.att$hasatt, strsplit(run.att$value, "[, \\(\\)\\{\\}:;]")[[1]][1], source.att$value))
  stopifnot(!is.null(run.name))

  project <- get.project(f)

  return(c(var=var.name, tres=time.res.cmip5, model=model.short.name, emissions=expt.name, run=run.name, trange=date.range, tstart=date.range[1], tend=date.range[2], institution=inst.name, project=project))
}
...
create.data.file.id <- function(f, filename, con) {
  ...
  query <- paste("INSERT INTO data_files(filename, run_id, first_1mib_md5sum, unique_id, time_set_id, x_dim_name, y_dim_name, z_dim_name, t_dim_name) VALUES(",
                 paste(shQuote(filename), run.id, shQuote(first.MiB.md5sum), shQuote(unique.id), time.set.id, x.dim.name, y.dim.name, z.dim.name, t.dim.name, sep=","),
...
update.data.file.id <- function(f, data.file.id, filename, con) {
  result <- dbSendQuery(con, paste("DELETE FROM ensemble_data_file_variables WHERE data_file_variable_id IN (SELECT data_file_variable_id FROM data_file_variables WHERE data_file_id=", data.file.id, ");"))
  result <- dbSendQuery(con, paste("DELETE FROM data_file_variables WHERE data_file_id=", data.file.id, ";"))
  result <- dbSendQuery(con, paste("DELETE FROM data_files WHERE data_file_id=", data.file.id, ";"))
...
update.data.file.index.time <- function(f, data.file.id, con) {
  return(dbSendQuery(con, paste("UPDATE data_files SET index_time = now() WHERE data_file_id =", data.file.id)))
}

update.data.file.filename <- function(f, data.file.id, new.filename, con) {
  return(dbSendQuery(con, paste("UPDATE data_files SET index_time = now(), filename =", shQuote(new.filename), " WHERE data_file_id =", data.file.id)))
}
...
get.data.file.id <- function(f, filename, con) {
  first.MiB.md5sum <- get.first.MiB.md5sum(filename)
  unique.id <- create.unique.id(f, filename)
  query <- paste("SELECT data_file_id,filename,first_1mib_md5sum,unique_id,index_time from data_files where first_1mib_md5sum='", first.MiB.md5sum, "' OR unique_id='", unique.id, "';", sep="")
  result <- dbSendQuery(con, query)
...
## Finds and returns the id of the data_file_variable which was used to compute the anomaly field if field is determined to be an anomaly field; otherwise returns NULL.
...
get.time.set.id <- function(f, con) {
  ...
  query <- paste("SELECT time_set_id FROM time_sets WHERE multi_year_mean =", multi.year.mean, "AND start_date=", shQuote(start.date), "AND end_date=", shQuote(end.date), "AND time_resolution=", shQuote(time.resolution), "AND num_times=", length(time.series), "AND calendar=", shQuote(cal))
  ##print(query)
  result <- dbSendQuery(con, query)
  time.set.id <- fetch(result, -1)
  
  if(nrow(time.set.id) == 0) {
    query <- paste("INSERT INTO time_sets(calendar, start_date, end_date, time_resolution, multi_year_mean, num_times) VALUES(", paste(shQuote(cal), shQuote(start.date), shQuote(end.date), shQuote(time.resolution), multi.year.mean, length(time.series), sep=","), ") RETURNING time_set_id")
    ##print(query)
    result <- dbSendQuery(con, query)
    ## Should check result
    ...
    time.bits <- paste(shQuote(format(time.series, "%Y-%m-%d %H:%M:%S")), seq_along(time.series) - 1, time.set.id, collapse="),(", sep=",")
    query <- paste("INSERT INTO times(timestep, time_idx, time_set_id) VALUES(", time.bits, ");" )
    ##print(query)
    ...
    if(!is.null(attr(time.series, "climatology.bounds"))) {
      ...
      query <- paste("INSERT INTO climatological_times(time_start, time_end, time_idx, time_set_id) VALUES(", do.call(paste, c(lapply(clim.bnds.list, function(x) { paste(x[1], x[2], x[3], time.set.id, sep=",") }), sep="),(")), ");", sep="")
      ##print(query)
      result <- dbSendQuery(con, query)
    }
...
get.level.set.id <- function(f, v, con) {
  ...
  query <- paste("SELECT level_set_id FROM levels NATURAL JOIN level_sets WHERE pressure_level IN (", paste(levels, collapse=",", sep=","), ") AND level_units = '", levels.dim$units, "' GROUP BY level_set_id HAVING count(vertical_level)=", length(levels), ";", sep="")
  ##print(query)
  result <- dbSendQuery(con, query)
  level.set.id <- fetch(result, -1)

  if(nrow(level.set.id) == 0) {
    query <- paste("INSERT INTO level_sets(level_units) values('", levels.dim$units, "') RETURNING level_set_id;", sep="")
    ##print(query)
    result <- dbSendQuery(con, query)
    ## Should check result
    ...
    level.bits <- do.call(paste, c(lapply(seq_along(levels), function(x) { return(paste(levels.bnds[x,1], levels.bnds[x,2], levels.bnds[x,3], x - 1, level.set.id, sep=",")) }), sep="),("))
    query <- paste("INSERT INTO levels(level_start, vertical_level, level_end, level_idx, level_set_id) VALUES(", level.bits, ");" )
    ##print(query)
...
get.variable.alias.id <- function(variable.standard.name, variable.long.name, variable.units, con) {
  query <- paste("SELECT variable_alias_id from variable_aliases where variable_long_name='", variable.long.name, "' and variable_standard_name='", variable.standard.name, "' and variable_units='", variable.units, "';", sep="")
  ##print(query)
  result <- dbSendQuery(con, query)
  variable.id <- fetch(result, -1)
  
  if(nrow(variable.id) == 0) {
    query <- paste("INSERT INTO variable_aliases(variable_long_name, variable_standard_name, variable_units) values(", paste(shQuote(variable.long.name), shQuote(variable.standard.name), shQuote(variable.units), sep=","), ") RETURNING variable_alias_id;", sep="")
    ##print(query)
    result <- dbSendQuery(con, query)
    variable.id <- fetch(result, -1)
...
get.srid <- function(f, v, con) {
  ...
  if(nrow(sr.id) == 0) {
    ...
    query <- paste("INSERT INTO spatial_ref_sys(srid, auth_name, auth_srid, srtext, proj4text) values(", paste(cur.srid, "\'PCIC\'", cur.srid, paste("'", wkt.string, "','", proj4.string, "'", sep=""), sep=","), ");", sep="")
    ##print(query)
    result <- dbSendQuery(con, query)
...
get.bnds.center.array <- function(f, d, bnds.var=NULL) {
  ...
  if(!is.null(bnds.var)) {
    ...
    half.steps.filled <- steps[c(1:floor(length(steps) / 2), ceiling(length(steps) / 2 + 0.001), ceiling(length(steps) / 2 + 0.001):length(steps))] / 2
    return(rbind(d$vals - half.steps.filled, d$vals, d$vals + half.steps.filled))
  }
}
...
get.grid.id <- function(f, v, con) {
  ...
  query <- paste("SELECT grid_id from grids where srid=", srid, " and ABS((xc_grid_step - ", xc.res, ") / xc_grid_step) < ", grid.diff.fraction, " and ABS((yc_grid_step - ", yc.res, ") / yc_grid_step) < ", grid.diff.fraction, " and ABS((xc_origin - ", xc.origin, ") / xc_origin) < ", grid.diff.fraction, " and ABS((yc_origin - ", yc.origin, ") / yc_origin) < ", grid.diff.fraction, " and xc_count=", xc.size, " and yc_count=", yc.size, " and evenly_spaced_y=", evenly.spaced.y, ";", sep="")
  ##print(query)
  result <- dbSendQuery(con, query)
  ...
  if(nrow(grid.id) == 0) {
    ...
    ## Get units
    query <- paste("INSERT INTO grids(srid, xc_grid_step, yc_grid_step, xc_origin, yc_origin, xc_count, yc_count, cell_avg_area_sq_km, evenly_spaced_y, xc_units, yc_units) VALUES(", paste(srid, xc.res, yc.res, xc.origin, yc.origin, xc.size, yc.size, cell.avg.area.sq.km, evenly.spaced.y, shQuote(x.dim$units), shQuote(y.dim$units), sep=","), ") RETURNING grid_id;", sep="")
    ##print(query)
    result <- dbSendQuery(con, query)
    grid.id <- fetch(result, -1)
    ...
    if(!evenly.spaced.y) {
      ...
      query <- paste("INSERT INTO y_cell_bounds(grid_id, top_bnd, y_center, bottom_bnd) VALUES(", do.call(paste, c(list(), apply(y.bnds, 2, function(x) { paste(grid.id, x[1], x[2], x[3], sep=",") }), sep="),(")), ");", sep="")
      ##print(query)
      result <- dbSendQuery(con, query)
    }
  }
...
## Finds and returns an emission_id if it can find a match; otherwise creates an emissions object and returns its emission_id.
get.emission.id <- function(f, con) {
  meta <- get.file.metadata(f)

  query <- paste("SELECT emission_id FROM emissions WHERE emission_short_name='", meta["emissions"], "';", sep="")
  ##print(query)
  result <- dbSendQuery(con, query)
  emission.id <- fetch(result, -1)
  if(nrow(emission.id) == 0) {
    query <- paste("INSERT INTO emissions(emission_short_name) VALUES('", meta["emissions"], "') RETURNING emission_id;", sep="");
    ##print(query)
    result <- dbSendQuery(con, query)
    emission.id <- fetch(result, -1)
  }
...
get.model.id <- function(f, con) {
  ...
  query <- paste("SELECT model_id FROM models WHERE model_short_name='", meta["model"], "';", sep="")
  ##print(query)
  result <- dbSendQuery(con, query)
  model.id <- fetch(result, -1)
  if(nrow(model.id) == 0) {
    query <- paste("INSERT INTO models(model_short_name, type, model_organization) VALUES(", paste(shQuote(meta["model"]), shQuote(model.type), shQuote(meta["institution"]), sep=","), ") RETURNING model_id;", sep="");
    ##print(query)
    result <- dbSendQuery(con, query)
    model.id <- fetch(result, -1)
  }
...
get.run.id <- function(f, con) {
  ...
  query <- paste("SELECT run_id FROM runs WHERE run_name='", meta["run"], "' and emission_id=", emission.id, " and model_id=", model.id, ";", sep="")
  ##print(query)
  result <- dbSendQuery(con, query)
  run.id <- fetch(result, -1)
  if(nrow(run.id) == 0) {
    query <- paste("INSERT INTO runs(run_name, emission_id, model_id, project) VALUES(", paste(shQuote(meta["run"]), emission.id, model.id, shQuote(meta["project"]), sep=","), ") RETURNING run_id;", sep="");
    ##print(query)
    result <- dbSendQuery(con, query)
    run.id <- fetch(result, -1)
  }
