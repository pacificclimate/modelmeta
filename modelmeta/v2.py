...
class ClimatologicalTime(Base):
    ...
    time_set_id = Column(
        Integer, ForeignKey("time_sets.time_set_id"), primary_key=True, nullable=False
    )
...
Index(
    "climatological_times_time_set_id_key", ClimatologicalTime.time_set_id, unique=False
)
...
class DataFile(Base):
    ...
    data_file_variables = relationship(
        "DataFileVariable", backref=backref("file", lazy="joined"), lazy="joined"
    )

    def __str__(self):
        return obj_repr(
            "id filename first_1mib_md5sum unique_id x_dim_name y_dim_name "
            "z_dim_name index_time run_id time_set_id",
            self,
        )
...
class DataFileVariable(Base):
    ...
    variable_alias_id = Column(
        Integer, ForeignKey("variable_aliases.variable_alias_id"), nullable=False
    )
    # VariableAlias defines backref `data_file_variables` in this class

    __mapper_args__ = {"polymorphic_identity": "none", "polymorphic_on": geometry_type}
...
class DataFileVariableDSGTimeSeries(DataFileVariable):
    ...
    id = Column(
        ...
        Integer,
        ForeignKey("data_file_variables.data_file_variable_id", ondelete="CASCADE"),
        primary_key=True,
        nullable=False,
    )
...
class DataFileVariablesQcFlag(Base):
    ...
    qc_flag_id = Column(
        Integer, ForeignKey("qc_flags.qc_flag_id"), primary_key=True, nullable=False
    )
...
UniqueConstraint(Ensemble.name, Ensemble.version, name="ensemble_name_version_key")
...
class Grid(Base):
    ...
    data_file_variables = relationship(
        "DataFileVariableGridded", backref=backref("grid")
    )
...
class Level(Base):
    ...
    level_set_id = Column(
        Integer, ForeignKey("level_sets.level_set_id"), primary_key=True, nullable=False
    )
...
class LevelSet(Base):
    ...
    data_file_variables = relationship(
        "DataFileVariableGridded", backref=backref("level_set")
    )
...
class Run(Base):
    ...
    files = relationship(
        "DataFile", backref=backref("run", lazy="joined"), lazy="joined"
    )

    def __repr__(self):
        return obj_repr(
            "id name project model_id emission_id "
            "driving_run_id initialized_from_id",
            self,
        )
...
class TimeSet(Base):
    ...
    climatological_times = relationship(
        "ClimatologicalTime", backref=backref("timeset")
    )
...
class VariableAlias(Base):
    ...
    # relation definitions
    data_file_variables = relationship(
        "DataFileVariable", backref=backref("variable_alias")
    )
...
class YCellBound(Base):
    ...
    grid_id = Column(
        Integer,
        ForeignKey(
            "grids.grid_id", name="y_cell_bounds_grid_id_fkey", ondelete="CASCADE"
        ),
        primary_key=True,
        nullable=False,
    )
