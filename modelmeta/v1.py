...
class Run(Base):
    ...
    ensembles = relationship(
        "EnsembleRun", backref=backref("run", lazy="joined"), lazy="joined"
    )
...
class Ensemble(Base):
    ...
    ensemble_runs = relationship(
        "EnsembleRun", backref=backref("ensemble"), lazy="joined"
    )
...
class Variable(Base):
    ...
    derived_from_id = Column(
        "derived_from", Integer, ForeignKey("variables.variable_id")
    )
...
class DataFileVariable(Base):
    ...
    anomaly_from_id = Column(
        "anomaly_from", Integer, ForeignKey("data_file_variables.data_file_variable_id")
    )

    anomaly_from = relationship(
        "DataFileVariable", foreign_keys="DataFileVariable.anomaly_from_id"
    )
